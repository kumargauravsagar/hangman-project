...
run=True
imag=pygame.image.load('Wins.png')
...
for i in range(0,7): 
	...
	imagee=pygame.transform.scale(image,(300,500))
...
def draw(): 
 ...
 # baground 
	size=pygame.transform.scale(imag,(1200,700))
	win.blit(size,(0,0))
	# TITLE for the game 

	title=TITLE.render("HangMan",1,(0,0,0,0)) 
	win.blit(title,(WIDTH/1.5 -title.get_width(), 8)) # Title in center and then y axis= 24 
	...
	for btn_pos in letters: 
		...
		if visible: 
			pygame.draw.circle(win,(0,0,0,0),(x,y),radius,5)
